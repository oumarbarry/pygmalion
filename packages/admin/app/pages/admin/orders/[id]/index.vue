...
const menuItems = computed(() => {
  ...
  if (!a) return []
  const rma = [
    a.canReturn ? { label: t('actionReturn'), icon: 'i-lucide-rotate-ccw', to: `${base}/return` } : null,
    a.canExchange ? { label: t('actionExchange'), icon: 'i-lucide-repeat', to: `${base}/exchange` } : null,
    a.canClaim ? { label: t('actionClaim'), icon: 'i-lucide-message-square-warning', to: `${base}/claim` } : null,
    a.canRefund ? { label: t('actionRefund'), icon: 'i-lucide-undo-2', to: `${base}/refund` } : null,
    a.canEdit ? { label: t('actionEditOrder'), icon: 'i-lucide-pencil', to: `${base}/edit` } : null,
  ].filter((x) => x !== null)
  const lifecycle = [
  ...
  >
    ...
    <div v-else class="flex flex-col gap-6">
      ...
      <UCard v-if="step" :ui="{ body: 'flex flex-col sm:flex-row sm:items-center gap-4 justify-between' }">
        ...
        <UButton v-else-if="step === 'ship'" size="lg" icon="i-lucide-truck" :label="t('actionShip')" :to="`${base}/ship`" />
        <UButton v-else-if="step === 'receiveReturn' && openReturn" size="lg" icon="i-lucide-package-open" :label="t('actionReceiveReturn')" :to="`${base}/returns/${openReturn.id}`" />
      ...
      <UCard>
        <template #header>
          <div class="flex flex-wrap items-center justify-between gap-3">
            ...
            <UButton v-if="abilities?.canShip" size="sm" icon="i-lucide-truck" :label="t('actionShip')" :to="`${base}/ship`" />
          </div>
        </template>
      ...
      <UCard v-if="returns.length || abilities?.canReturn">
        <template #header>
          <div class="flex flex-wrap items-center justify-between gap-3">
            ...
            <UButton v-if="abilities?.canReturn" size="sm" color="neutral" variant="outline" icon="i-lucide-rotate-ccw" :label="t('actionReturn')" :to="`${base}/return`" />
          </div>
        </template>
        ...
        <ul v-else class="flex flex-col divide-y divide-default">
          <li v-for="r in returns" :key="r.id" class="flex flex-col sm:flex-row sm:items-center gap-3 py-3">
            ...
            <div class="flex flex-wrap gap-2">
              <UButton size="sm" color="neutral" variant="outline" class="pyg-tap-target" icon="i-lucide-package-open" :label="returnAbilities(r).canReceive ? t('actionReceiveReturn') : t('actionOpenReturn')" :to="`${base}/returns/${r.id}`" />
              <UButton
