...
function rowActions(product: AdminProduct) {
  return [[
    { label: t('edit'), icon: 'i-lucide-pencil', to: `/admin/products/${product.id}` },
    {
...
<template>
  <PygPage :title="t('sectionProducts')" :description="t('productsSubtitle')">
    <template #actions>
      <UButton to="/admin/products/new" icon="i-lucide-plus" size="md" :label="t('newProduct')" />
    </template>
    ...
    <PygList v-else :items="products" :columns="columns" :loading="loading" :to="(p) => `/admin/products/${p.id}`">
