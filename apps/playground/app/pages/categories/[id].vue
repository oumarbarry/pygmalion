...
useSeoMeta({
  title: () => data.value?.category.name ?? t('productCategoryFallback'),
...
<template>
  <div class="mx-auto max-w-6xl px-4 py-10 sm:px-6 sm:py-14">
    ...
    <h1 class="shop-display mt-4 text-3xl text-highlighted sm:text-4xl">{{ data?.category.name ?? t('productCategoryFallback') }}</h1>
    <p v-if="data?.category.description" class="shop-prose mt-2 text-muted">{{ data.category.description }}</p>

    <nav v-if="children.length" class="mt-6 flex flex-wrap gap-2" :aria-label="t('productSubcategories')">
      <UButton
        v-for="c in children"
        :key="c.id"
        :to="`/categories/${c.id}`"
    ...
    <ProductGrid
      ...
      :pending="pending"
      :error="error"
      :empty-title="t('productCategoryEmptyTitle')"
      :empty-message="t('productCategoryEmptyMessage')"
      @retry="refresh()"
    />
  </div>
</template>
