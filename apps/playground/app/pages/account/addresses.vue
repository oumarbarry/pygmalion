...
<template>
  <section>
    <div class="flex flex-wrap items-center justify-between gap-3">
      <h2 class="text-lg font-bold text-highlighted">Mes adresses</h2>
      <UButton
        v-if="editing !== 'new'"
        color="primary"
        size="sm"
    ...
    <form v-if="editing" class="mt-6 space-y-4 rounded-xl border border-default p-5" @submit.prevent="save">
      <UFormField label="Nom de l'adresse" hint="Maison, Bureau…">
        <UInput v-model="form.addressName" class="w-full" data-testid="address-name" />
      </UFormField>
      <div class="grid gap-4 sm:grid-cols-2">
        <UFormField label="Prénom"><UInput v-model="form.firstName" autocomplete="given-name" class="w-full" /></UFormField>
        <UFormField label="Nom"><UInput v-model="form.lastName" autocomplete="family-name" class="w-full" /></UFormField>
      </div>
      ...
      <UFormField label="Complément"><UInput v-model="form.address2" class="w-full" /></UFormField>
      <div class="grid gap-4 sm:grid-cols-[10rem_1fr]">
        <UFormField label="Code postal"><UInput v-model="form.postalCode" autocomplete="postal-code" class="w-full" /></UFormField>
      ...
      <UFormField label="Pays" hint="code à 2 lettres" required>
        <UInput v-model="form.countryCode" maxlength="2" required class="w-full sm:max-w-24" data-testid="address-country" />
      </UFormField>
      <UFormField label="Téléphone"><UInput v-model="form.phone" type="tel" autocomplete="tel" class="w-full sm:max-w-xs" /></UFormField>

      <div class="flex gap-2">
        <UButton type="submit" color="primary" :loading="busy" label="Enregistrer" data-testid="save-address" />
    ...
    <AsyncState
      ...
      empty-message="Ajoutez-en une et le checkout la proposera automatiquement."
      ...
      <ul class="mt-6 grid gap-4 sm:grid-cols-2">
        <li v-for="a in addresses" :key="a.id" class="rounded-xl border border-default p-5" data-testid="address-card">
          ...
          <div v-if="confirmingDelete === a.id" class="mt-4 rounded-lg bg-muted p-3">
            <p class="text-sm text-highlighted">Supprimer cette adresse ?</p>
            <div class="mt-2.5 flex gap-2">
              <UButton size="sm" class="min-h-11" color="error" :loading="busy" label="Supprimer" data-testid="confirm-delete" @click="destroy(a.id)" />
              <UButton size="sm" class="min-h-11" color="neutral" variant="ghost" label="Annuler" @click="confirmingDelete = null" />
            </div>
          </div>
          <div v-else class="mt-4 flex gap-2">
            <UButton size="sm" class="min-h-11" color="neutral" variant="outline" label="Modifier" @click="open(a)" />
            <UButton size="sm" class="min-h-11" color="neutral" variant="ghost" label="Supprimer" @click="confirmingDelete = a.id" />
          </div>
        </li>
      </ul>
    </AsyncState>
